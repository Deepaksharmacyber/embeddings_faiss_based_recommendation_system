...
def hybrid_recommend(top_k=5):
    ...
    # ---------------------------
    # Build USER embedding dynamically
    # ---------------------------
    ...
    for a in user_activity["activities"]:
        ...
        user_vector += weight * course_embeddings[a["course_id"] - 1]
        total_weight += weight
    ...
    for c in candidates:
        ...
        c["popularity"] = popularity.get(cid, {}).get("enrollments", 0)
...
if __name__ == "__main__":
    recos = hybrid_recommend()
    print("\n📌 Final Hybrid Recommendations:")
    for r in recos:
        print(f"- {r['course']['title']} (score={round(r['final_score'], 3)})")
